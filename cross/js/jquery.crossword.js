...
(function ($) {
	$.fn.crossword = function (entryData) {
		/*
				Qurossword Puzzle: un crucigrama javascript + jQuery
"luz" se refiere a una caja blanca - o una entrada

NOTAS DE DEV:
- activePosition y activeClueIndex son los vars principales que configuran la interfaz de usuario cada vez que hay una interacción
- 'Entrada' es un término de rompecabezas utilizado para describir el grupo de entradas de letras que representan una solución de palabras
- Este rompecabezas no está diseñado para ocultar de forma segura a los respondedores. Un usuario puede ver los contestadores en la fuente js
- Se puede agregar una provisión xhr más tarde para llegar a un punto final en keyup para verificar el respondedor
- El ordenamiento de la serie de problemas no importa. Las propiedades de posición y orientación son suficiente información.
- Los autores de los rompecabezas deben proporcionar las coordenadas x, y iniciales para cada entrada.
- La orientación de la entrada debe proporcionarse en lugar de las coordenadas x, y provistas de finalización (la secuencia de comandos podría ajustarse para usar los extremos x, y de terminación)
- Las respuestas se proporcionan mejor en minúsculas, y NO pueden tener espacios - agregará soporte para eso más adelante
			*/
